<#
.SYNOPSIS
This script is a part of Nishang. It copies a file from an NTFS partitioned volume by reading the raw volume and parsing the NTFS structures. This bypasses file DACL's,
read handle locks, and SACL's. You must be an administrator (elevated privileges) to run the script. This can be used to read SYSTEM files which are normally
locked, such as the NTDS.dit file or registry hives. 

.DESCRIPTION
The script can copy files off an NTFS volume by opening a read handle to the entire volume (such as c:) and parsing the NTFS structures. This requires you
...
.\Invoke-NinjaCopy.ps1 -path c:\windows\system32\config\system -localdestination c:\test\system
